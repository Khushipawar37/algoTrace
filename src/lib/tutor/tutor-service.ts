...
export async function generateTutorResponse(input: { message: string; problem: TutorProblemContext; hintLevel: TutorHintLevel; intent: StudentHelpIntent; code?: string; history: TutorModelMessage[] }) {
  ...
  return generateValidatedText({ hintLevel: input.hintLevel, generate: async (retry) => generateTutorText(buildTutorSystemPrompt({ hintLevel: input.hintLevel, problem: input.problem, codeProvided: Boolean(input.code), retry }), messages) });
}
