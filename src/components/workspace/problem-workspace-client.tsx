...
export function ProblemWorkspaceClient({slug}:{slug:string}) {
  ...
  async function execute(kind:"run"|"submit"){
    ...
    try{const response=await fetch(`/api/problems/${encodeURIComponent(slug)}/${kind}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({language:"cpp",code,...(kind==="run"?{input}:{})})});setResult(await response.json() as ResultDto);}
